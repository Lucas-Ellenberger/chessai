...
class Reference(edq.util.json.DictConverter):
    ...
    1) `chessai.util.reflection.Reference` -- a fully qualified name.
    2) `chessai/util/reflection.py:Reference` -- a path and class name,
    """
    ...
    def parse_string(text: str, check_alias: bool = True) -> tuple[str | None, str | None, str]:
        ...
        if (check_alias and ('.' not in text)):
            text = chessai.util.alias.lookup(text, text)
